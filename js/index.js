$(function(){
	...
	function Helix(){
		...
		$('#main li').each(function(i){
			$(this).css({
				'transform' : 'rotateY('+ 10*i +'deg) translateY('+ (firsttY+tY*i) +'px) translateZ(800px)'
			});
		})
	}

	function Sphere(){
		var arr = [2,5,7,10,11,14,21,16,13,9,8,6,3];
		...
		$('#main li').each(function(j){
			...
			$(this).css({
				transform : 'rotateY('+y+'deg) rotateX('+x+'deg) rotateZ('+z+'deg) translateZ(700px)'
			});
		});
	}
	...
	(function(){
		var $mainLi = $('#main li');
		var $show = $('#show');
		var $img = $("#show .s-img img");
		var $title = $("#show .s-title h2");
		var $author = $("#show .s-author h2");
		var $dat = $("#show .s-dec #date");
		var $intod = $("#show .s-dec #introd"); 
		var sum;
		$mainLi.click(function(ev){
			...
			}).find($img).attr('src',demoArr[sum].img);
			$author.html(demoArr[sum].author);
			
			$dat.html(demoArr[sum].date1);
			$intod.html(demoArr[sum].test);
			$title.html(demoArr[sum].kt);
			ev.stopPropagation();
		});
		...
		$show.click(function(ev){
			...
			$('#frame').show().animate({
				left : 0
			},1000).find('iframe').attr('src' ,demoArr[sum].src);
			ev.stopPropagation();
		});
